...
def test_topology_groups_vlan_gateway_infrastructure_and_endpoints(client, monkeypatch):
    ...
    devices = [
        ("Building gateway", "172.2.4.1", "Router"),
        ("Floor switch", "172.2.4.2", "Switch"),
        ("Lab workstation", "172.2.4.20", "Workstation"),
    ]
    ...
    group = body["groups"][0]
    assert group["label"] == "VLAN LAB-20 · 172.2.4.0/24"
    assert group["gateway_ip"] == "172.2.4.1"
