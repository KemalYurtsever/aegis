...
NUCLEI_RECORD = {
    ...
    "matcher-name": "dotenv",
    "matched-at": "http://198.18.1.25:8080/.env?token=must-not-be-stored",
    "info": {
...
def test_nuclei_parser_keeps_bounded_evidence_for_the_registered_target():
    ...
    observations = parse_nuclei_jsonl(
        output,
        expected_ip="198.18.1.25",
        targets=("http://198.18.1.25:8080",),
    )
    ...
    assert finding.matched_at == "http://198.18.1.25:8080/.env"
...
def test_web_targets_use_detected_protocols_and_enforce_per_host_limit():
    ...
    targets, skipped = web_targets("198.18.1.25", open_ports)

    assert "https://198.18.1.25:443" in targets
    assert "http://198.18.1.25:8080" in targets
