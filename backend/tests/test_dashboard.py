...
def test_dashboard_aggregates_devices_and_events(client, monkeypatch):
    first = create_device(client, "Alpha", "192.168.56.10")
    create_device(client, "Beta", "192.168.56.11", is_active=False)
...
def test_combined_dashboard_refresh_reuses_one_device_snapshot(client, monkeypatch):
    device = create_device(client, "Shared state", "192.168.56.20")
    monkeypatch.setattr(
        "app.services.monitoring_service.check_ip",
