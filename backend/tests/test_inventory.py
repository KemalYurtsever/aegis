...
def test_dhcp_import_adds_and_updates_by_mac_without_overwriting_manual_name(client):
    ...
    assert devices[0]["ip_address"] == "192.168.50.21"
...
def test_dhcp_import_reports_mac_and_ip_ownership_conflict(client):
    ...
    first = client.post(
        ...
        headers=headers,
        json={"rows": [
            {"ip_address": "10.10.10.10", "mac_address": "00:11:22:33:44:55"},
            {"ip_address": "10.10.10.11", "mac_address": "00:11:22:33:44:66"},
        ]},
    )
    ...
    conflict = client.post(
        "/api/inventory/dhcp-leases/import",
        headers=headers,
        json={"rows": [{"ip_address": "10.10.10.11", "mac_address": "00:11:22:33:44:55"}]},
    )
...
def test_inventory_health_finds_lease_monitoring_and_duplicate_mac_issues(client):
    ...
    with client.app.state.session_factory() as db:
        expired = Device(
            name="Expired lease", ip_address="192.168.80.10", mac_address="00:11:22:33:44:55",
            device_type="Other", is_active=True, lease_expires_at=now - timedelta(hours=1),
        )
        expiring = Device(
            name="Expiring lease", ip_address="192.168.80.11", mac_address="00:11:22:33:44:55",
