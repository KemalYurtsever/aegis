...
def test_device_crud(client):
    ...
    assert body["ip_address"] == "198.18.56.20"
...
def test_asset_metadata_is_normalized_and_validated(client):
    ...
    invalid = create_device(client, ip_address="198.18.56.21", criticality="URGENT")
...
def test_device_tags_are_normalized_and_persisted(client):
    ...
    invalid = create_device(client, ip_address="198.18.56.21", tags=["not@valid"])
    assert invalid.status_code == 422
...
def test_device_persists_across_database_reconnection(tmp_path):
    ...
    with first_session_factory() as session:
        session.add(Device(name="Persistent Device", ip_address="198.18.56.50", device_type="Server"))
        session.commit()
    ...
    with restarted_session_factory() as session:
        restored = session.scalar(select(Device).where(Device.ip_address == "198.18.56.50"))
