...
def test_discovery_does_not_import_subnet_network_or_broadcast_addresses(client, admin_headers, monkeypatch):
    ...
    monkeypatch.setattr(
        "app.services.mdns_service.discover_mdns",
        lambda *_args: {
            "198.18.1.0": {"services": ["http"]},
            "198.18.1.60": {"services": ["googlecast"]},
            "198.18.1.255": {"services": ["http"]},
        },
    )
    ...
    assert addresses == {"198.18.1.50", "198.18.1.60"}
...
def test_arp_table_ignores_broadcast_and_multicast_mac_addresses(monkeypatch):
    monkeypatch.setattr(
        ...
        lambda *_args, **_kwargs: SimpleNamespace(
            returncode=0,
            stdout=(
                "  198.18.1.20    02-11-22-33-44-55    dynamic\n"
                "  198.18.1.255   ff-ff-ff-ff-ff-ff    static\n"
