...
def test_scheduler_checks_only_active_devices(tmp_path, monkeypatch):
    ...
    with sessions() as db:
        db.add_all(
            [
                Device(name="Active", ip_address="198.18.56.10", device_type="Server", is_active=True),
                Device(name="Inactive", ip_address="198.18.56.11", device_type="Server", is_active=False),
            ]
        )
        db.commit()
...
def test_scheduler_creates_and_resolves_offline_agent_alert(tmp_path, monkeypatch):
    engine, sessions = make_session_factory(tmp_path)
    with sessions() as db:
        device = Device(name="Agent host", ip_address="198.18.56.30", device_type="Server", is_active=True)
