...
DEVICE = {"name": "Lab server", "ip_address": "192.168.1.50", "device_type": "Server", "is_active": True}
...
def test_attack_surface_records_bounded_nuclei_template_evidence(client, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda *_args: NucleiValidationResult(
            targets=("http://192.168.1.50:8080",),
            observations=(NucleiObservation(
                ...
                matched_at="http://192.168.1.50:8080/.env",
                matcher_name="dotenv",
    ...
    assert finding["validation_target"] == "http://192.168.1.50:8080/.env"
