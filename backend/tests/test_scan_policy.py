...
LOCAL_NETWORKS = (ip_network("192.168.10.0/24"),)
...
def test_scan_policy_rejects_active_subnet_network_and_broadcast_addresses():
    assert "network address" in scan_target_rejection_reason("192.168.10.0", LOCAL_NETWORKS)
    assert "broadcast address" in scan_target_rejection_reason("192.168.10.255", LOCAL_NETWORKS)
    assert scan_target_rejection_reason("192.168.10.20", LOCAL_NETWORKS) is None
...
def test_host_membership_excludes_ipv4_network_and_broadcast_addresses():
    assert is_host_in_network("192.168.10.20", "192.168.10.0/24") is True
    assert is_host_in_network("192.168.10.0", "192.168.10.0/24") is False
    assert is_host_in_network("192.168.10.255", "192.168.10.0/24") is False
    assert is_host_in_network("192.168.11.20", "192.168.10.0/24") is False
    assert is_host_in_network("192.168.10.0", "192.168.10.0/31") is True
    assert is_host_in_network("malformed", "192.168.10.0/24") is False
    assert is_host_in_network("2001:db8::10", "192.168.10.0/24") is False
...
def test_boolean_scan_policy_wraps_the_rejection_reason(monkeypatch):
    ...
    assert scan_target_allowed("192.168.10.20") is True
    assert scan_target_allowed("192.168.10.255") is False
