...
def test_nmap_top_1000_scan_uses_managed_docker_syn_scan(monkeypatch):
    ...
    result = scan_nmap_top_tcp_ports("198.18.1.10")
...
def test_local_fallback_scans_the_nmap_ranked_top_1000(monkeypatch):
    ...
    assert len(result.scanned_ports) == 1000
    assert captured == {"target": "198.18.1.10", "ports": list(NMAP_TOP_1000_TCP_PORTS)}
    assert [item.port for item in result.open_ports] == [80]
...
def test_service_detection_uses_fast_version_probe_on_open_ports(monkeypatch):
    ...
    services = scan_nmap_service_versions("198.18.1.10", [443, 80, 80])
