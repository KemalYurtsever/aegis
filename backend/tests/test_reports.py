...
def test_availability_report_calculates_uptime_incidents_and_streaks(client):
    ...
    with client.app.state.session_factory() as db:
        primary = Device(
            name="Critical server", ip_address="192.168.100.10", device_type="Server", criticality="CRITICAL"
        )
        empty = Device(name="Unchecked switch", ip_address="192.168.100.20", device_type="Switch")
