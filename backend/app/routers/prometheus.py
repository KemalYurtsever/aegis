...
def _expected_token() -> str:
    direct = os.getenv("LIIMS_PROMETHEUS_TOKEN", "").strip()
    token_file = os.getenv("LIIMS_PROMETHEUS_TOKEN_FILE", "").strip()
    if direct:
        return direct
    if token_file:
        try:
...
def prometheus_metrics(request: Request, authorization: str | None = Header(default=None)) -> PlainTextResponse:
    ...
    lines = [
        "# HELP liims_devices_total Number of registered devices.",
        "# TYPE liims_devices_total gauge",
    ]
    with request.app.state.session_factory() as db:
        devices = list(db.scalars(select(Device).order_by(Device.id)))
        lines.append(f"liims_devices_total {len(devices)}")
        lines.extend(["# HELP liims_device_status Device status (1 online, 0 offline, -1 unknown).", "# TYPE liims_device_status gauge"])
        lines.extend(["# HELP liims_device_latency_milliseconds Latest device latency.", "# TYPE liims_device_latency_milliseconds gauge"])
        for device in devices:
            ...
            lines.append(f"liims_device_status{{{labels}}} {status}")
            if result is not None and result.latency_ms is not None:
                lines.append(f"liims_device_latency_milliseconds{{{labels}}} {result.latency_ms}")
        active_alerts = db.scalar(select(func.count(AlertEvent.id)).where(AlertEvent.resolved_at.is_(None))) or 0
        lines.extend(["# HELP liims_active_alerts Number of unresolved alerts.", "# TYPE liims_active_alerts gauge", f"liims_active_alerts {active_alerts}"])
        lines.extend(["# HELP liims_service_status Latest service status (1 up, 0 down, -1 unknown).", "# TYPE liims_service_status gauge"])
        for check in db.scalars(select(ServiceCheck).order_by(ServiceCheck.id)):
            ...
            lines.append(f"liims_service_status{{{labels}}} {-1 if result is None else (1 if result.status == 'UP' else 0)}")
        lines.extend(["# HELP liims_host_resource_percent Latest host resource utilization.", "# TYPE liims_host_resource_percent gauge"])
        for device in devices:
            ...
            if metric:
                for resource, value in (("cpu", metric.cpu_percent), ("memory", metric.memory_percent), ("disk", metric.disk_percent)):
                    lines.append(f'liims_host_resource_percent{{device_id="{device.id}",device_name="{_label(device.name)}",resource="{resource}"}} {value}')
    scheduler = request.app.state.monitor_scheduler
    lines.extend(["# HELP liims_scheduler_running Whether automatic monitoring is running.", "# TYPE liims_scheduler_running gauge", f"liims_scheduler_running {1 if scheduler.is_running else 0}", "# EOF"])
