...
from app.services.snmp_service import SNMP_COMMUNITY_ENV, get_or_create_config, poll_device

router = APIRouter(prefix="/api/devices/{device_id}/snmp", tags=["snmp"], dependencies=[Depends(require_admin)])

def serialize_config(config: SnmpConfig) -> SnmpConfigRead:
    allowed = config.community_env == SNMP_COMMUNITY_ENV
    return SnmpConfigRead(id=config.id, device_id=config.device_id, enabled=config.enabled and allowed, port=config.port, community_env=SNMP_COMMUNITY_ENV, secret_configured=allowed and bool(os.getenv(SNMP_COMMUNITY_ENV, "").strip()), updated_at=config.updated_at)
